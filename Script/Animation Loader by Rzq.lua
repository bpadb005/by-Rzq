...
local function backupAnimation(character)
    ...
    backupAnimationDesc = Instance.new("HumanoidDescription")
    backupAnimationDesc.WalkAnimation = desc.WalkAnimation
    backupAnimationDesc.RunAnimation = desc.RunAnimation
    backupAnimationDesc.JumpAnimation = desc.JumpAnimation
    backupAnimationDesc.IdleAnimation = desc.IdleAnimation
    backupAnimationDesc.FallAnimation = desc.FallAnimation
    backupAnimationDesc.ClimbAnimation = desc.ClimbAnimation
    backupAnimationDesc.SwimAnimation = desc.SwimAnimation

    pcall(function()
        backupAnimationDesc:SetEmotes(desc:GetEmotes())
        backupAnimationDesc:SetEquippedEmotes(desc:GetEquippedEmotes())
    end)
end
...
local function loadAnimation(username)
    ...
    if not humanoidDesc then
        return false, "Gagal mendapatkan animasi dari " .. username
    end
    ...
    pcall(function()
        currentDesc:SetEmotes(humanoidDesc:GetEmotes())
        currentDesc:SetEquippedEmotes(humanoidDesc:GetEquippedEmotes())
    end)

    if not pcall(function()
        humanoid:ApplyDescriptionClientServer(currentDesc)
        task.wait(0.5)
    end) then
        return false, "Animasi gagal diubah ke: " .. username
    end

    return true, "Animasi berhasil diubah ke: " .. username
end
...
titleLabel.Text = "Animation Loader\nby Rzq"
...
restoreButton.Text = "Restore Animation"
...
statusLabel.Text = "Masukkan username ￫ klik Load Animation."
statusLabel.Parent = contentContainerFrame

loadButton.MouseButton1Click:Connect(function()
    statusLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
    statusLabel.Text = "Memuat animasi..."
    task.spawn(function()
        local success, message = loadAnimation(usernameBox.Text)
...
restoreButton.MouseButton1Click:Connect(function()
    if not backupAnimationDesc then
        statusLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
        statusLabel.Text = "Animasi asli belum tersimpan."
        return
    end

    statusLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
    statusLabel.Text = "Mengembalikan animasi asli..."
    ...
    task.spawn(function()
        local success = pcall(function()
            ...
            if humanoid then
                ...
                currentDesc.RunAnimation = backupAnimationDesc.RunAnimation
                currentDesc.JumpAnimation = backupAnimationDesc.JumpAnimation
                currentDesc.IdleAnimation = backupAnimationDesc.IdleAnimation
                currentDesc.FallAnimation = backupAnimationDesc.FallAnimation
                currentDesc.ClimbAnimation = backupAnimationDesc.ClimbAnimation
                currentDesc.SwimAnimation = backupAnimationDesc.SwimAnimation

                pcall(function()
                    currentDesc:SetEmotes(backupAnimationDesc:GetEmotes())
                    currentDesc:SetEquippedEmotes(backupAnimationDesc:GetEquippedEmotes())
                end)
        ...
        statusLabel.TextColor3 = success and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100)
        statusLabel.Text = success and "Animasi asli berhasil dikembalikan." or "Gagal mengembalikan animasi."
    end)
end)
